...
This example demonstrates how to use the SPI (Serial Peripheral Interface) module to communicate with an external SPI device on the Texas Instruments C2000 Piccolo LaunchPad Evaluation Kit (LAUNCHHXL-F28027F).
...
2. **Create a New Project**:
    ...
    - Enter a project name, e.g., `SPI_Communication`.
...
5. **Run the Program**:
    - Click the `Run` button to start the program. The SPI module will transmit data to an external SPI device.
...
#### InitSpi
This function configures the SPI module to operate in master mode with a specified baud rate.

#### SpiTransmit
This function sends data to an SPI slave device.

#### main
The main function initializes the system control, GPIO, and SPI, then enters an infinite loop where it transmits data to an SPI slave every second.
