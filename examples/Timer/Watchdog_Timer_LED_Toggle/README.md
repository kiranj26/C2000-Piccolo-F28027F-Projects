# Watchdog Timer LED Toggle Example

This example demonstrates how to use the Watchdog Timer (WDT) to toggle an LED on the Texas Instruments C2000 Piccolo LaunchPad Evaluation Kit (LAUNCHHXL-F28027F).
...
2. **Create a New Project**:
    ...
    - Enter a project name, e.g., `Watchdog_Timer_LED_Toggle`.
...
5. **Run the Program**:
    - Click the `Run` button to start the program. The LED should start toggling periodically.
...
#### InitWatchdog
This function configures the Watchdog Timer to generate periodic interrupts.

#### main
The main function initializes the system control and GPIO, configures the PIE control registers and vector table, maps the watchdog ISR, and enables the necessary interrupts. It then enters an infinite loop where it services the watchdog timer and delays for 1 second.
